...
internal sealed class FileWatcherApp(string configPath, IProcessRunner? processRunner = null) : IDisposable
{
    ...
    internal readonly ConcurrentDictionary<string, CancellationTokenSource> _pendingTokens = new(StringComparer.OrdinalIgnoreCase);
    internal readonly ConcurrentDictionary<string, FileSystemWatcher> _directoryWatchers = new(StringComparer.OrdinalIgnoreCase);
    ...
    private async Task RunConsoleLoopAsync(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            if (Console.KeyAvailable)
            {
                ...
                {
                    case 'r': await ReloadConfigurationAsync(token); break;
                    case 'q': throw new OperationCanceledException(token);
    ...
    internal void SetupWatchers()
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(entry.Source) || !File.Exists(entry.Source)) continue;
    ...
    private FileSystemWatcher CreateWatcher(string directory)
    {
        ...
        watcher.Error += (_, e) => LogService.Log(LogLevel.Error, $"Watcher error in {directory}: {e.GetException().Message}");
        return watcher;
    }
    ...
    internal void HandleFileEvent(FileSystemEventArgs args)
    {
        ...
        if (directory == null || !_directoryEntries.TryGetValue(directory, out var entries)) return;
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                if (entry.CopyTo != null)
                {
                    ...
                    LogService.Log(LogLevel.Copy, $"Copied {Path.GetFileName(entry.Source)} to {entry.CopyTo}");
                }
            ...
            catch (Exception ex) { LogService.Log(LogLevel.Error, $"Error processing {entry.Source}: {ex.Message}"); }
            finally { _pendingTokens.TryRemove(entry.Source, out _); cts.Dispose(); }
        });
    }
    ...
    internal async Task RunHookAsync(string command, string location, CancellationToken token)
    {
        ...
        try
        {
            ...
            var exitCode = await _processRunner.RunAsync(
                ...
                token);
            if (exitCode != 0) LogService.Log(LogLevel.Warning, $"Hook exited with code {exitCode}");
        }
        catch (OperationCanceledException) { }
        catch (Exception ex) { LogService.Log(LogLevel.Error, $"Hook failed: {ex.Message}"); }
    }
    ...
    {
        foreach (var w in _directoryWatchers.Values) w.Dispose();
    ...
    {
        if (_disposed) return;
        foreach (var cts in _pendingTokens.Values) { cts.Cancel(); cts.Dispose(); }
        DisposeWatchers();
        _disposed = true;
